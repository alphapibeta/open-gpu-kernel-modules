...
MODULE_NAME="sincospif_gpu_taylor"
DEVICE_NAME="sincospif_gpu_taylor_dev"
...
load_module() {
    if ! lsmod | grep -q "$MODULE_NAME"; then
        ...
        # Check if the device file is created
        sleep 1
...
# Ensure the module is unloaded before starting the tests
unload_module
...
echo "Testing Taylor series version..."
load_module
run_test
analyze_perf
unload_module
